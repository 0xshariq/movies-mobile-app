...
const Signup = () => {
  ...
  const createAccount = async (data: FormData) => {
    ...
    try {
      ...
      const usersCollection = process.env.EXPO_PUBLIC_APPWRITE_USERS_COLLECTION_ID || 'users'
      if (databaseId) {
        await databases.createDocument(databaseId, usersCollection, 'unique()', {
          appwriteId: user.$id,
          email: data.email,
          name: data.name
        })
  ...
  <TextInput className="border border-gray-300 rounded-md p-3 mb-4" placeholder="Your name" value={name} onChangeText={(v) => { setName(v); setValue('name', v) }} />
  {errors.name ? <Text className="text-red-500 mb-2">{errors.name.message}</Text> : null}
  ...
  {errors.password ? <Text className="text-red-500 mb-2">{errors.password.message}</Text> : null}
      ...
      <Pressable className={`py-3 rounded-md items-center ${loading ? 'bg-gray-400' : 'bg-blue-600'}`} onPress={handleSubmit(createAccount)} disabled={loading}>
        <Text className="text-white font-medium">{loading ? 'Creating...' : 'Create account'}</Text>
      </Pressable>
    </View>
  )
}
